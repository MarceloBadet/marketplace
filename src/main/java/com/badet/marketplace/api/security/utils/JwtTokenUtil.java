package com.badet.marketplace.api.security.utils;
...
public class JwtTokenUtil {
	...
	public String refreshToken(String token) {
		String refresedToken;
		try{
			Claims claims = getClaimsFromToken(token);
			claims.put(CLAIM_KEY_CREATED, new Date());
			refresedToken = gerarToken(claims);
		}catch (Exception e) {
			refresedToken = null;
		}
	...
	/**
	 * Verifica e retorna se um token JWT a válido.
	 * 
	 * @param token
	 * @return boolean
	 */	
	...
	private boolean tokenExpirado(String token) {
		Date dataExpirada = this.getExpirationDateFromToken(token);
		if(dataExpirada == null) {
			return false;
		}
		return dataExpirada.before(new Date());
	}
